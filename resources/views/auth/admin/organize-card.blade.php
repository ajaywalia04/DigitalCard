    ...
    <div class="dashboard-main container">
        ...
        <main id="dashboardMainContent" class="main-content rounded-lg shadow-md">
            ...
            <div class="bg-white p-6 rounded-lg shadow-sm">
                <form action="{{ route('admin.dashboard.orgainze.card.store',['mycard'=>$mycard->uuid]) }}" method="POST" id="notesTagsEditForm" class="space-y-6">
                    @csrf
                    <div>
                        <label for="contactNotes" class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                        <textarea id="contactNotes" name="notes" rows="6" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Add your notes about this contact...">{{ $user->cardnote->first() ? $user->cardnote->first()->note:'' }}</textarea>
                    </div>
                            ...
                            <div id="categoriesSelection" class="flex flex-wrap gap-2 p-2 border border-gray-300 rounded-md bg-gray-50">
                                @foreach($user->category as $category)
                                    <div>
                                        ...
                                        {{ $categoryIds == optional($category)->id ? 'checked' : '' }}  name="category" class="select-item-radio">
                    ...
                    <div class="flex justify-end space-x-3 mt-6">
                        <a href="{{ route('admin.dashboard.shared.card.view') }}" type="button" id="cancelNotesTagsEdit" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md font-semibold hover:bg-gray-300 transition-colors duration-200">Cancel</a>
