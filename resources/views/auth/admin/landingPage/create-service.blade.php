    ...
    <div class="dashboard-main container">
        ...
        <main id="dashboardMainContent" class="main-content rounded-lg shadow-md">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-3xl font-bold text-gray-900 mb-6">Create Your Service</h1>
            ...
            <div class="card-form-container bg-white rounded-xl shadow-2xl overflow-hidden p-8">
                ...
                <form action="{{ route('dashboard.landing.service.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea id="sub_heading" name="sub_heading"
                                class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200" rows="3" placeholder="Defining clear product roadmaps and strategic initiatives to align with business goals and market needs.">{{ old('subheading') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="icon" name="icon" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200">
                            <option value="1" {{ old('icon') == '1' ? 'selected' : '' }}>💡</option>
                            <option value="2" {{ old('icon') == '2' ? 'selected' : '' }} >🎨</option>
                            <option value="3" {{ old('icon') == '3' ? 'selected' : '' }} >📊</option>
                            <option value="4" {{ old('icon') == '4' ? 'selected' : '' }}>🛠️</option>
                            <option value="5" {{ old('icon') == '5' ? 'selected' : '' }}>🚀</option>
                            <option value="6" {{ old('icon') == '6' ? 'selected' : '' }}>🤝</option>
                        </select>
                    </div>
