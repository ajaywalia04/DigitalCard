    ...
    <div class="dashboard-main container">
        ...
        <main id="dashboardMainContent" class="main-content rounded-lg shadow-md">
            <div class="card-form-container bg-white rounded-xl shadow-2xl overflow-hidden p-8">
                ...
                <form action="{{ route('dashboard.card.update',['myCard'=>$myCard->uuid]) }}" method="POST" class="space-y-6">
                    @csrf
                    <!-- Personal Information -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="full_name">Full Name</label>
                            <input type="text" id="full_name" name="full_name" value="{{ old('full_name', $myCard->full_name) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="email" id="email" name="email" value="{{ $myCard->email }}"  readonly
                             class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200"  placeholder="jane.doe@example.com">
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea id="company_address" name="company_address" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200" rows="3" placeholder="123 Business Rd, Suite 456, City, State 12345">{{ old('company_address', $myCard->company_address) }}</textarea>
                    </div>
