...
<body>
    <div class="card-container bg-white rounded-xl shadow-2xl overflow-hidden transform transition-all duration-300 hover:scale-105 relative">
        ...
        @endif
        <div class="relative bg-gray-700 h-32 flex items-center justify-center">
            <?php
                $initials = strtoupper(collect(explode(' ', trim($myCard->full_name)))
                        ...
                        ->implode(''));
                ?>
            <img src={{ "https://placehold.co/100x100/374151/FFFFFF?text=".$initials }} alt="Profile Picture" class="w-24 h-24 rounded-full border-4 border-white shadow-lg -mb-12 relative z-10 object-cover">
        </div>

        <div class="text-center pt-16 pb-8 px-6">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ $myCard->full_name}}</h1>
            <p class="text-lg text-gray-700 font-medium mb-2">{{ $myCard->job_title}}</p>
            ...
            <div class="space-y-3 text-left max-w-xs mx-auto">
                <div class="flex items-center text-gray-700">
                    ...
                    <a href="mailto:jane.doe@example.com" class="text-gray-700 hover:text-gray-900 transition-colors duration-200">{{ $myCard->email }}</a>
                </div>
                <div class="flex items-center text-gray-700">
                    <span class="icon">&#128222;</span>
                    <a href="tel:+1234567890" class="text-gray-700 hover:text-gray-900 transition-colors duration-200">+91 {{ $myCard->phone_no}}</a>
                </div>
                <div class="flex items-center text-gray-700">
                    <span class="icon">&#128205;</span>
        ...
        <div class="px-6 pb-6 pt-4">
            <form method="Post" action="{{ route('my.card.accept',['myCard'=>$myCard->slug]) }}">
                @csrf
                <button type="submit" class="w-full bg-gray-600 text-white py-3 rounded-md font-semibold hover:bg-gray-700 transition-colors duration-200 shadow-md mb-2">
                    Accept Card
                </button>
            </form>
