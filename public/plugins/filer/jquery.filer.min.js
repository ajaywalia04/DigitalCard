	...
	(a.fn.filer = function (b) {
				...
				m = {
					...
					_bindInput: function () {
						l.changeInput && h.length > 0 && h.on("click", m._clickHandler),
							...
							l.dragDrop &&
								(l.dragDrop.dragContainer.on("drag dragstart dragend dragover dragenter dragleave drop", function (a) {
									a.preventDefault(), a.stopPropagation();
								}),
								l.dragDrop.dragContainer.on("drop", m._dragDrop.drop),
								l.dragDrop.dragContainer.on("dragover", m._dragDrop.dragEnter),
								l.dragDrop.dragContainer.on("dragleave", m._dragDrop.dragLeave)),
							l.uploadFile && l.clipBoardPaste && a(window).on("paste", m._clipboardPaste);
					},
					...
					_changeInput: function () {
						...
						(e.prop("jFiler").newInputEl = h),
							l.dragDrop && (l.dragDrop.dragContainer = l.dragDrop.dragContainer ? a(l.dragDrop.dragContainer) : h),
							(!l.limit || (l.limit && l.limit >= 2)) && (e.attr("multiple", "multiple"), "[]" != e.attr("name").slice(-2) ? e.attr("name", e.attr("name") + "[]") : null),
							e.attr("disabled") || l.disabled ? ((l.disabled = !0), m._unbindInput(!0), g.addClass("jFiler-disabled")) : ((l.disabled = !1), m._bindInput(), g.removeClass("jFiler-disabled")),
										...
										(g.onload = function () {
											...
											if (((e.height = a), (e.width = c), g.width < e.width || g.height < e.height || l <= 1)) {
												...
												var o = document.createElement("canvas"),
													p = o.getContext("2d");
												(o.width = 0.5 * g.width), (o.height = 0.5 * g.height), (p.fillStyle = "#fff"), p.fillRect(0, 0, o.width, o.height), p.drawImage(g, 0, 0, o.width, o.height), p.drawImage(o, 0, 0, 0.5 * o.width, 0.5 * o.height), f.drawImage(o, k > e.width ? k - e.width : 0, 0, 0.5 * o.width, 0.5 * o.height, 0, 0, k, j);
											}
					...
					_ajax: {
						...
						progressHandling: function (a, b) {
							if (a.lengthComputable) {
								...
								null != l.uploadFile.onProgress && "function" == typeof l.uploadFile.onProgress ? l.uploadFile.onProgress(c, b, i, g, h, e) : null,
									b
								...
								for (var e = 0; e < c.length; e++)
									if (c[e].type.indexOf("image") !== -1 || c[e].type.indexOf("text/uri-list") !== -1) {
										...
										(f.name = Math.random().toString(36).substring(5)),
											(f.name += f.type.indexOf("/") != -1 ? "." + f.type.split("/")[1].toString().toLowerCase() : ".png"),
											m._onChange(a, [f]),
					...
					_onSelect: function (b) {
						l.uploadFile &&
							...
							m.files[b]._pendRemove && (m._itFc.html.hide(), m._remove(null, { binded: !0, data: { id: m._itFc.id } })),
							null != l.onSelect && "function" == typeof l.onSelect ? l.onSelect(m.files[b], m._itFc.html, i, g, h, e) : null,
							b + 1 >= m.files.length && (null != l.afterShow && "function" == typeof l.afterShow ? l.afterShow(i, g, h, e) : null);
					},
					_onChange: function (b, c) {
						...
						else for (var f = 0; f < m.files.length; f++) (m.files[f]._choosed = !0), m._addToMemory(f), m._onSelect(f);
					},
					...
					_remove: function (b, d) {
						...
						var f = function (b) {
							...
							j = function (b, c) {
								...
								f(e),
									m._itFl.splice(c, 1),
									m._itFl.length < 1 ? (m._reset(), m._clear()) : m._set("feedback", m._itFl.length + " " + l.captions.feedback2),
					...
					_addToMemory: function (b) {
						m._itFl.push({ id: m._itFl.length, file: m.files[b], html: a(), ajax: !1, uploaded: !1 }), (l.addMore || m.files[b]._appended) && (m._itFl[m._itFl.length - 1].input = e), (m._itFc = m._itFl[m._itFl.length - 1]), (e.prop("jFiler").files_list = m._itFl), (e.prop("jFiler").current_file = m._itFc);
					},
					_assets: {
								...
								case "string":
									return b.replace(/\{\{fi-(.*?)\}\}/g, function (a, b) {
										return (
											(b = b.replace(/ /g, "")),
											b.match(/(.*?)\|limitTo\:(\d+)/)
												? b.replace(/(.*?)\|limitTo\:(\d+)/, function (a, b, d) {
														var b = c[b] ? c[b] : "",
															e = b.substring(0, d);
		...
		(a.fn.filer.defaults = {
			...
			captions: {
				...
				errors: { filesLimit: "Only {{fi-limit}} files are allowed to be uploaded.", filesType: "Only Images are allowed to be uploaded.", filesSize: "{{fi-name}} is too large! Please upload file up to {{fi-fileMaxSize}} MB.", filesSizeAll: "Files you've choosed are too large! Please upload files up to {{fi-maxSize}} MB.", folderUpload: "You are not allowed to upload folders." },
			},
		});
})(jQuery);
